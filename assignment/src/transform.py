...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = data.merge(orders[['order_id', 'order_status']], on="order_id", how="left")
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_ammount_per_date.rename(columns={"order_purchase_timestamp": "date"}, inplace=True)
    order_purchase_ammount_per_date["date"] = pd.to_datetime(order_purchase_ammount_per_date["date"])
    
    holidays["date"] = pd.to_datetime(holidays["date"])
    
    order_purchase_ammount_per_date["holiday"] = order_purchase_ammount_per_date["date"].isin(
        holidays["date"]
    )
